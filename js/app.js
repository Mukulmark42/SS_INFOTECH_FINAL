...
const App = (() => {
  ...
  // ── Offline IFSC Directory Fallback ─────────────────────────
  const OFFLINE_IFSC_MAP = {
  ...
  function onPresumptiveSectionChange() {
    const sec = _v('f-presumptive-section') || '44AD';
    const is44ADA = sec === '44ADA';
    const sub = document.getElementById('panel-sec-sub');
    const infoText = document.getElementById('sec-info-text');
    if (sub) sub.textContent = is44ADA ? '(Section 44ADA - Professionals)' : '(Section 44AD - Business)';
    if (infoText) {
      infoText.innerHTML = is44ADA
        ? 'Under <strong>Section 44ADA (Professionals)</strong>, deemed profit is declared at <strong>50%</strong> of gross receipts. Gross receipts will be auto-calculated from your declared income.'
        : 'Under <strong>Section 44AD (Business)</strong>, profit is automatically declared at <strong>20%</strong> of turnover. Turnover will be auto-calculated from your declared business income.';
    }
    recalcIncome();
  }
  ...
  function generateAckNoField() {
    const pan = _v('f-pan') || 'ABCDE1234F';
    const filingDate = _v('f-filing-date') || '';
    const ay = _v('f-ay') || '2025-26';
    const compNo = _peekCompNumber();
    const ackNo = ReportAck.getAckNo({ pan, filingDate, ay }, compNo);
    _setVal('f-ack-no', ackNo);
    return ackNo;
  }
  ...
  function generateReport() {
    ...
    if (!data.client.ackNo || String(data.client.ackNo).replace(/\D/g, '').length !== 15) {
  ...
  function _saveClientRecord(data) {
    ...
    const record = {
      ...
      compNo:             data.compNo,
      ...data.client,
      presumptiveSection: data.computation?.presumptiveSection || _v('f-presumptive-section') || '44AD',
      regime:             _v('f-regime') || 'New',
      banks:              data.banks,
  ...
  function _renderClientTable(data) {
    ...
    if (grid) {
      grid.innerHTML = clients.map((c, i) => {
        ...
        return `
          <div class="client-card ${isChecked ? 'selected-card' : ''}" id="client-card-${c.id}">
            <div class="cc-header">
              <div class="d-flex align-items-center gap-2">
            ...
            <div class="cc-body">
              ...
              <div class="cc-row"><span class="cc-label">Section</span><span class="cc-val">${_esc(c.presumptiveSection || (c.nature ? '44AD' : '—'))}</span></div>
              ...
              <div class="cc-row"><span class="cc-label">Ack No</span><span class="cc-val" style="font-size:10px;color:var(--text-muted);font-family:monospace;">${_esc(c.ackNo || '—')}</span></div>
    ...
    _updateBulkSelectionUI(clients);
  ...
  function toggleClientSelect(id, isChecked) {
    if (isChecked) {
  ...
  function toggleSelectAllClients(checked) {
    const clients = DB.search(document.getElementById('clientSearch')?.value || '');
    if (checked) {
      clients.forEach(c => selectedClientIds.add(c.id));
    } else {
      selectedClientIds.clear();
    }
    _renderClientTable(clients);
  }

  function _updateBulkSelectionUI(currentClients) {
    const clients = currentClients || DB.search(document.getElementById('clientSearch')?.value || '');
    ...
    if (totalBadge) {
      totalBadge.textContent = count > 0 ? `${count} of ${clients.length} selected` : `${clients.length} clients total`;
    }
    if (selectAllChk && clients.length > 0) {
      const allSelected = clients.every(c => selectedClientIds.has(c.id));
      selectAllChk.checked = allSelected;
      selectAllChk.indeterminate = count > 0 && !allSelected;
    }
  }
  ...
  function exportClientsCSV() {
    const ids = selectedClientIds.size > 0 ? Array.from(selectedClientIds) : null;
    const csvContent = DB.exportCSV(ids);
    if (!csvContent) {
    ...
    a.href = url;
    a.download = `SS_INFOTECH_Clients_${dateStr}.csv`;
  ...
  function editClient(id) {
    ...
    setTimeout(() => {
      ...
      _setSelect('f-presumptive-section', c.presumptiveSection || '44AD');
      _setSelect('f-regime',  c.regime || 'New');
      onPresumptiveSectionChange();
  ...
  function updateProjectStorageStats() {
    const usage = DB.getStorageUsage ? DB.getStorageUsage() : { totalKB: 0, maxMB: 5.0, pctUsed: 0, clientCount: 0, slipCompanyCount: 0, slipRecordCount: 0, statementRecordCount: 0 };
    ...
    if (elItr) elItr.textContent = usage.clientCount;
    if (elStmt) elStmt.textContent = usage.statementRecordCount;
    if (elSlip) elSlip.textContent = usage.slipRecordCount;
    if (elComp) elComp.textContent = usage.slipCompanyCount;

    const elBar = document.getElementById('store-progress-bar');
    const elTxt = document.getElementById('store-usage-text');
    if (elBar) {
      elBar.style.width = `${Math.min(100, usage.pctUsed)}%`;
      if (usage.pctUsed > 85) {
        elBar.className = 'progress-bar bg-danger';
      } else if (usage.pctUsed > 60) {
        elBar.className = 'progress-bar bg-warning';
      } else {
        elBar.className = 'progress-bar bg-primary';
      }
    }
    if (elTxt) {
      elTxt.textContent = `${usage.totalKB} KB / ${usage.maxMB.toFixed(1)} MB (${usage.pctUsed}% used)`;
    }
  }
  ...
  async function fetchBankDetailsByIFSC(ifscCode, options = {}) {
    ...
    try {
      ...
      console.warn('Razorpay IFSC API error, checking offline fallback directory:', err);
      const bankPrefix = ifsc.slice(0, 4);
      const fallbackBank = OFFLINE_IFSC_MAP[bankPrefix];

      if (fallbackBank) {
        const fallbackData = {
          BANK: fallbackBank,
          BRANCH: `MAIN BRANCH (${ifsc.slice(-4)})`,
          IFSC: ifsc,
          CITY: '',
          STATE: ''
        };
        ...
        if (options.bankNameElId) {
          ...
          if (nameEl) {
            if (nameEl.tagName === 'SELECT') {
              let opt = Array.from(nameEl.options).find(o => o.value.toUpperCase().includes(fallbackBank) || fallbackBank.includes(o.value.toUpperCase()));
              if (opt) nameEl.value = opt.value;
            } else {
              nameEl.value = fallbackBank;
            }
          }
        }
        if (options.branchElId) {
          const branchEl = document.getElementById(options.branchElId);
          if (branchEl && !branchEl.value) branchEl.value = fallbackData.BRANCH;
        }
        if (options.codeElId) {
          const codeEl = document.getElementById(options.codeElId);
          if (codeEl) codeEl.value = ifsc.slice(-4);
        }
        if (statusEl) {
          statusEl.innerHTML = `<span class="text-success fw-semibold"><i class="bi bi-check-circle-fill"></i> ${fallbackBank} (Offline Verified)</span>`;
        }
        return fallbackData;
      }
